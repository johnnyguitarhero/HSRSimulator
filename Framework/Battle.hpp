...
{
public:
	Battle(): m_turn(0), m_timer(0.0f)
	{
		m_actionQueue.clear();
		m_mobileEntityMap.clear();
		m_pTeam = nullptr;
	...
	void InitBattle(Team* pTeam, int turn)
	{
		...
		{
			m_mobileEntityMap.insert({ m_actionQueue[i]->m_name, m_actionQueue[i] });
		}
	}
...
private:
	...
	std::unordered_map<std::string, MobileEntity*> m_mobileEntityMap; // Map from mobile entity name to the pointer
